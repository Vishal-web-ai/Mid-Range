...
export function CustomSelect({ value, onChange, options, placeholder = "Select...", className }: CustomSelectProps) {
  ...
  return (
    <div ref={ref} className={cn("relative", className)}>
      ...
      {open && (
        <div className="border-steel-gray bg-dark-grey absolute z-50 mt-1 max-h-60 w-full overflow-auto rounded border shadow-lg [scrollbar-width:none] [&::-webkit-scrollbar]:hidden" data-lenis-prevent>
          {options.map((option) => (
            <button
              key={option.value}
              type="button"
